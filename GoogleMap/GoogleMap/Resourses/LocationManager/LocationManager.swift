...
final class LocationManager: NSObject {
    ...
    typealias GoogleReverseGeoLocationClosure = ((_ placemark:PlacemarkResults?,_ error: Error?)->Void)

    private var geoLocationCompletionHandler: ReverseGeoLocationClosure?
    private var googleGeoLocationCompletionHandler: GoogleReverseGeoLocationClosure?
    ...
    private var googleReverseGeocoding = false
...
//MARK:- Public Methods
    ...
    func getGoogleReverseGeoCodedLocation(location:CLLocation,completionHandler:@escaping GoogleReverseGeoLocationClosure) {
        let session = URLSession(configuration: URLSessionConfiguration.default)
        let request = URLRequest(url: URL(string: "https://maps.googleapis.com/maps/api/geocode/json?latlng=\(location.coordinate.latitude),\(location.coordinate.longitude)&key=\(APIKey.mapKey)")!)
        session.dataTask(with: request) { data, response, error in
            ...
            do {
                let loca = try JSONDecoder().decode(Geocode.self, from: data)
                completionHandler(loca.results?.first,nil)
            }catch(let error) {
                completionHandler(nil, error)
            }
